from utils.utils_deeplift import *
from utils.evalation import metrics,metrics_link
from utils.utils_link import *

class convex_link():
    def __init__(self,Hold,Hnew,W,Hnew_mlp, Hold_mlp,W_mlp,goal_1,goal_2,index_new,index_old,\
                 layernumbers,hidden,dataset,graph_new,graph_old,addgoalpath_1,addgoalpath_2\
                 ,removegoalpath_1,removegoalpath_2, topk_pathlist,edges_new,edges_old,model,addedgelist,removeedgelist):
        self.Hold=Hold
        self.Hnew=Hnew
        self.goal_1=goal_1
        self.goal_2 = goal_2
        self.Hnew_mlp=Hnew_mlp
        self.Hold_mlp=Hold_mlp
        self.layernumbers=layernumbers
        self.index_new=index_new
        self.index_old=index_old
        self.hidden=hidden
        self.dataset=dataset
        self.W=W
        self.W_mlp=W_mlp
        self.graph_new=graph_new
        self.graph_old=graph_old
        self.addgoalpath_1=addgoalpath_1
        self.addgoalpath_2=addgoalpath_2
        self.removegoalpath_1=removegoalpath_1
        self.removegoalpath_2=removegoalpath_2
        self.topk_pathlist=topk_pathlist
        self.edges_new = edges_new
        self.edges_old = edges_old
        self.model=model
        self.addedgelist=addedgelist
        self.removeedgelist=removeedgelist


    def contribution_value(self):
        goal_1=self.goal_1
        goal_2 = self.goal_2
        layernumbers=self.layernumbers
        Hold = self.Hold
        Hnew = self.Hnew
        W = self.W
        feature=self.dataset[0].x
        hidden=self.hidden
        addgoalpath_1=self.addgoalpath_1
        addgoalpath_2=self.addgoalpath_2
        removegoalpath_1=self.removegoalpath_1
        removegoalpath_2=self.removegoalpath_2
        nclass = 2
        Hold_mlp=self.Hold_mlp
        Hnew_mlp=self.Hnew_mlp
        W_mlp=self.W_mlp

        addedgelist=self.addedgelist
        removeedgelist=self.removeedgelist
        index_new=self.index_new
        index_old = self.index_old






        deepliftresultma = np.zeros((len(addgoalpath_1) + len(addgoalpath_2) + len(removegoalpath_1) + len(removegoalpath_2), nclass))
        for i in range(0, nclass):

            deeplift_numpy = mlp_linear(layernumbers, Hold_mlp, Hnew_mlp, W_mlp, hidden, i, index_old,
                                              index_new)
            deeplift_numpy = deeplift_numpy.sum(axis=0)
            add_deepliftresult_1, add_deeplift_1 = deepliftrelumultigaijin_link(goal_1, 0, layernumbers,
                                                                                 addgoalpath_1,
                                                                                 Hnew, Hold, \
                                                                                 addedgelist, W, feature.numpy(),
                                                                                 hidden, deeplift_numpy,
                                                                                 'start',
                                                                                 hidden)
            # print(deeplift_1)
            # print(sum(deepliftresult_1.values()))
            add_deepliftresult_2, add_deeplift_2 = deepliftrelumultigaijin_link(goal_2, 0, layernumbers,
                                                                                 addgoalpath_2,
                                                                                 Hnew, Hold, \
                                                                                 addedgelist, W, feature.numpy(),
                                                                                 hidden, deeplift_numpy, 'end',
                                                                                 hidden)
            remove_deepliftresult_1, remove_deeplift_1 = deepliftrelumultigaijin_link(goal_1, 0, layernumbers,
                                                                                       removegoalpath_1,
                                                                                       Hold, Hnew, \
                                                                                       removeedgelist, W,
                                                                                       feature.numpy(),
                                                                                       hidden, deeplift_numpy,
                                                                                       'start',
                                                                                       hidden)
            remove_deepliftresult_2, remove_deeplift_2 = deepliftrelumultigaijin_link(goal_2, 0, layernumbers,
                                                                                       removegoalpath_2,
                                                                                       Hold, Hnew, \
                                                                                       removeedgelist, W,
                                                                                       feature.numpy(),
                                                                                       hidden, deeplift_numpy,
                                                                                       'end',
                                                                                       hidden)
            # print(deeplift_1)
            deepliftresult_1 = dict()
            for key, value in add_deepliftresult_1.items():
                deepliftresult_1[key] = value
            for key, value in remove_deepliftresult_1.items():
                deepliftresult_1[key] = -value

            deepliftresult_2 = dict()
            for key, value in add_deepliftresult_2.items():
                deepliftresult_2[key] = value
            for key, value in remove_deepliftresult_2.items():
                deepliftresult_2[key] = -value

            deepliftresult = dict()
            for key, value in deepliftresult_1.items():
                if key in deepliftresult_2.keys():
                    deepliftresult[key] = value + deepliftresult_2[key]
                if key not in deepliftresult_2.keys():
                    deepliftresult[key] = value
            for key, value in deepliftresult_2.items():
                if key not in deepliftresult_1.keys():
                    deepliftresult[key] = value

            for j in range(0, len(addgoalpath_2)):
                strpath = []
                for pathindex in addgoalpath_2[j]:
                    strpath.append(str(pathindex))
                c = ','.join(strpath)
                deepliftresultma[j][i] = deepliftresult[c]
            for j in range(len(addgoalpath_2), len(addgoalpath_2) + len(addgoalpath_1)):
                strpath = []
                for pathindex in addgoalpath_1[j - len(addgoalpath_2)]:
                    strpath.append(str(pathindex))
                c = ','.join(strpath)
                deepliftresultma[j][i] = deepliftresult[c]
            for j in range(len(addgoalpath_2) + len(addgoalpath_1),
                           len(removegoalpath_2) + len(addgoalpath_2) + len(addgoalpath_1)):
                strpath = []
                for pathindex in removegoalpath_2[j - (len(addgoalpath_2) + len(addgoalpath_1))]:
                    strpath.append(str(pathindex))
                c = ','.join(strpath)
                deepliftresultma[j][i] = deepliftresult[c]

            for j in range(len(removegoalpath_2) + len(addgoalpath_2) + len(addgoalpath_1),
                           len(removegoalpath_2) + len(addgoalpath_2) + len(addgoalpath_1) + len(removegoalpath_1)):
                strpath = []
                for pathindex in removegoalpath_1[
                    j - (len(addgoalpath_2) + len(addgoalpath_1) + len(removegoalpath_2))]:
                    strpath.append(str(pathindex))
                c = ','.join(strpath)
                deepliftresultma[j][i] = deepliftresult[c]
        return deepliftresultma





    def select_importantpath(self,Hold,Hnew,removeedgelist,addedgelist):
        topk_pathlist=self.topk_pathlist
        goal_logits_mask_list=[]
        goal_logits_add_list=[]

        goal_1 = self.goal_1
        goal_2 = self.goal_2
        layernumbers = self.layernumbers
        Hold = self.Hold
        Hnew = self.Hnew
        W = self.W
        feature = self.dataset[0].x
        hidden = self.hidden
        addgoalpath_1 = self.addgoalpath_1
        addgoalpath_2 = self.addgoalpath_2
        removegoalpath_1 = self.removegoalpath_1
        removegoalpath_2 = self.removegoalpath_2
        nclass = 2
        Hold_mlp = self.Hold_mlp
        Hnew_mlp = self.Hnew_mlp
        W_mlp = self.W_mlp


        index_new = self.index_new
        index_old = self.index_old


        number = softmax(Hnew_mlp[1][index_new]).T.dot(
            Hnew_mlp[1][index_new] - Hold_mlp[1][index_old])
        # print('number', number)
        number2 = 0
        for j in range(Hnew_mlp[1].shape[1]):
            number2 = number2 + np.exp(Hnew_mlp[1][index_new][j])
        number3 = softmax(Hold_mlp[1][index_new]).T.dot(
            -Hnew_mlp[1][index_new] + Hold_mlp[1][index_old])
        number4 = 0
        for j in range(Hnew_mlp[1].shape[1]):
            number4 = number4 + np.exp(Hold_mlp[1][index_new][j])
        print('number2',number2)
        print('number4', number4)

        deepliftresultma = self.contribution_value()


        for l in range(0, len(topk_pathlist)):
            print('l', l)

            topk_path = topk_pathlist[l]
            choicelist_mask = main_linear_mask(number3, number4, topk_path, index_new, index_old, deepliftresultma, nclass,
                                            Hnew_mlp,
                                            Hold_mlp,
                                            layernumbers)


            if choicelist_mask != [] and choicelist_mask is not None:
                choiceresultdict = dict()
                for i in range(0, len(choicelist_mask)):
                    if i < len(addgoalpath_2):
                        strpath = []
                        for pathindex in addgoalpath_2[i]:
                            strpath.append(str(pathindex))
                        c = ','.join(strpath)
                        choiceresultdict[c] = choicelist_mask[i]
                    if i >= len(addgoalpath_2) and i < len(addgoalpath_2) + len(addgoalpath_1):
                        strpath = []
                        for pathindex in addgoalpath_1[i - len(addgoalpath_2)]:
                            strpath.append(str(pathindex))
                        c = ','.join(strpath)
                        choiceresultdict[c] = choicelist_mask[i]
                    if i >= len(addgoalpath_2) + len(addgoalpath_1) and i < len(addgoalpath_2) + len(
                            removegoalpath_2) + len(addgoalpath_1):
                        strpath = []
                        for pathindex in removegoalpath_2[i - len(addgoalpath_2) - len(addgoalpath_1)]:
                            strpath.append(str(pathindex))
                        c = ','.join(strpath)
                        choiceresultdict[c] = choicelist_mask[i]
                    if i >= len(addgoalpath_2) + len(removegoalpath_2) + len(addgoalpath_1):
                        strpath = []
                        for pathindex in removegoalpath_1[
                            i - len(addgoalpath_2) - len(addgoalpath_1) - len(removegoalpath_2)]:
                            strpath.append(str(pathindex))
                        c = ','.join(strpath)
                        choiceresultdict[c] = choicelist_mask[i]

                sortchoiceresultdict = sorted(choiceresultdict.items(), key=lambda item: item[1], reverse=True)
                pa_add = []
                pa_remove = []

                for i in range(0, topk_path):
                    deepliftpath = []
                    s1 = sortchoiceresultdict[i][0].split(',')
                    for j in s1:
                        deepliftpath.append(int(j))
                    if deepliftpath in addgoalpath_1 or deepliftpath in addgoalpath_2:
                        pa_add.append(deepliftpath)
                    if deepliftpath in removegoalpath_1 or deepliftpath in removegoalpath_2:
                        pa_remove.append(deepliftpath)
                # print('len(pa_add)',len(pa_add))
                # print('len(pa_remove)',len(pa_remove))

                edges_new = self.edges_new
                model = self.model
                goal_logits_mask = metrics_link(model, feature, self.goal_1, self.goal_2, pa_add, pa_remove, edges_new,
                                                self.hidden,Hnew,'mask',removeedgelist,addedgelist).cal()
                goal_logits_mask_list.append(goal_logits_mask)



            choicelist = main_con_linear(number, number2, topk_path, index_new, index_old, deepliftresultma,
                                               nclass,
                                               Hnew_mlp,
                                               Hold_mlp,
                                               layernumbers)
            choiceresultdict = dict()
            for i in range(0, len(choicelist)):
                if i < len(addgoalpath_2):
                    strpath = []
                    for pathindex in addgoalpath_2[i]:
                        strpath.append(str(pathindex))
                    c = ','.join(strpath)
                    choiceresultdict[c] = choicelist[i]
                if i >= len(addgoalpath_2) and i < len(addgoalpath_2) + len(addgoalpath_1):
                    strpath = []
                    for pathindex in addgoalpath_1[i - len(addgoalpath_2)]:
                        strpath.append(str(pathindex))
                    c = ','.join(strpath)
                    choiceresultdict[c] = choicelist[i]
                if i >= len(addgoalpath_2) + len(addgoalpath_1) and i < len(addgoalpath_2) + len(
                        removegoalpath_2) + len(addgoalpath_1):
                    strpath = []
                    for pathindex in removegoalpath_2[i - len(addgoalpath_2) - len(addgoalpath_1)]:
                        strpath.append(str(pathindex))
                    c = ','.join(strpath)
                    choiceresultdict[c] = choicelist[i]
                if i >= len(addgoalpath_2) + len(removegoalpath_2) + len(addgoalpath_1):
                    strpath = []
                    for pathindex in removegoalpath_1[
                        i - len(addgoalpath_2) - len(addgoalpath_1) - len(removegoalpath_2)]:
                        strpath.append(str(pathindex))
                    c = ','.join(strpath)
                    choiceresultdict[c] = choicelist[i]

            sortchoiceresultdict = sorted(choiceresultdict.items(), key=lambda item: item[1], reverse=True)
            # print('sortchoiceresultdict',sortchoiceresultdict)
            pa_add = []
            pa_remove = []

            for i in range(0, topk_path):
                deepliftpath = []
                s1 = sortchoiceresultdict[i][0].split(',')
                for j in s1:
                    deepliftpath.append(int(j))
                if deepliftpath in addgoalpath_1 or deepliftpath in addgoalpath_2:
                    pa_add.append(deepliftpath)
                if deepliftpath in removegoalpath_1 or deepliftpath in removegoalpath_2:
                    pa_remove.append(deepliftpath)

            edges_new=self.edges_new
            model=self.model
            feature = self.dataset[0].x
            # goal_logits_mask = metrics_link(model, feature, self.goal_1,self.goal_2, pa_add, pa_remove, edges_new,self.hidden).cal()
            # goal_logits_mask_list.append(goal_logits_mask)
            goal_logits_add = metrics_link(model, feature,  self.goal_1,self.goal_2,pa_add, pa_remove, self.edges_old,self.hidden,Hold,'add',removeedgelist,addedgelist).cal()
            goal_logits_add_list.append(goal_logits_add)
            # print('mask',goal_logits_mask)
            # print('add', goal_logits_add)
        return goal_logits_mask_list, goal_logits_add_list





